---
title: "Stat Tests as Linear Models"
...
abstract: This course is a part of the `R for Artists and Designers` at the School of Foundation Studies, Srishti Manipal Institute of Art, Design, and Technology,Bangalore. Most, if not all, student projects in Srishti's Final Year require primary research and data gathering to derive insights based on which to create design interventions. This course is intended to assist with making statistically significant insights that drive design decisions. This document deals with the basics of stats. The method followed is that of Jonas Lindoloev, wherein every stat test is treated as a linear model y = mx + c.
editor_options: 
...
4.  [Jeffrey Walker "Elements of Statistical Modeling for Experimental Biology"](https://www.middleprofessor.com/files/applied-biostatistics_bookdown/_book/)
4.  Text: Diez, David M & Barr, Christopher D & Çetinkaya-Rundel, Mine:
    [OpenIntro Statistics](https://www.openintro.org/book/os/)
5.  Modern Statistics with R: From wrangling and exploring data to
...
p1 <- ggplot(mydata_long,aes(x = group, y = value)) +
  geom_jitter(width = 0.02, height = 0,aes(colour = group)) +
...
The net *area* of all the shaded squares is minimized in the caculation
...
where each of the $\beta_i$ are slopes defining the relationship
between y and $x_i$. Together, the RHS of that equation defines an
n-dimensional *plane*.

As per Lindoloev, many statistical tests, going from one-sample t-tests
to two-way ANOVA, are special cases of this system.

Also see [Jeffrey Walker "A linear-model-can-be-fit-to-data-with-continuous-discrete-or-categorical-x-variables"](https://www.middleprofessor.com/files/applied-biostatistics_bookdown/_book/intro-linear-models.html#a-linear-model-can-be-fit-to-data-with-continuous-discrete-or-categorical-x-variables)
...
Notes: 
1. When ranks are used, the slope of the linear model ($\beta_1$)
has the same value as the correlation coefficient ( $\rho$ ). 
2. Note that the slope from the linear model now has an intuitive
interpretation: **the number of ranks y changes for each change in rank
of x**. ( Ranks are "independent" of `sd` )
